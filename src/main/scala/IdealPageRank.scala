...
object IdealPageRank {
    ...
    //Output = args[2] ("hdfs://honolulu:30541/PA3Top10Ideal/")
    
    
        //Create spark SparkSession and spark context
        val spark = SparkSession.builder.appName("Ideal Page Rank").getOrCreate()
        ...
        val links = linkData.map(x => (x.split(": ")(0), x.split(": ")(1).split(" +"))).cache()
